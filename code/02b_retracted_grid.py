...
def main():
    os.makedirs("results", exist_ok=True); os.makedirs("results/act", exist_ok=True)
    ...
    for frac, seed in runs:
        ...
        np.save(f"{ACTDIR}/act_f{frac:.1f}_s{seed}.npy", A[:, keep, :].astype(np.float32))
        X = A[:, keep, :]
